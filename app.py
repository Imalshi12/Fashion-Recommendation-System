...
@login_required
def dashboard():
    # Fetch predictions related to the current user
...
def predict():
    ...
    if request.method == 'POST':
        ...
        # Create and save prediction with user_id
...
# Route for viewing fashion recommendations by body shape
...
# Admin route for managing fashion items
...
def manage_fashion():
    if request.method == 'POST':
        ...
        flash('Fashion item added successfully!')
...
# Route for editing fashion items
...
def edit_fashion_item(item_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Fashion item updated successfully!')
        return redirect(url_for('manage_fashion'))
    
    return render_template('edit_fashion_item.html', item=item)

# Route for deleting fashion items
...
def delete_fashion_item(item_id):
    ...
    db.session.commit()
    flash('Fashion item deleted successfully!')
    return redirect(url_for('manage_fashion'))
...
def add_to_cart(item_id):
    ...
    if existing_item:
        existing_item.quantity += 1  # Increase the quantity if already in the cart
    else:
    ...
    return redirect(url_for('fashion_recommendation', body_shape='Banana'))  # Redirect to a relevant page
...
# Route for checking out and displaying the payment form
...
# Route to process payment and show success message
@app.route('/cart/process_payment', methods=['POST'])
@login_required
def process_payment():
    # Get payment details from the form (in a real app, you'd validate these)
    name = request.form['name']
    ...
    # Simulate a payment process (this is just a mock)
    cart_items = CartItem.query.filter_by(user_id=current_user.id).all()
...
def update_profile():
    ...
    if form.validate_on_submit():
        ...
        # If the user wants to update their password, hash and store it
